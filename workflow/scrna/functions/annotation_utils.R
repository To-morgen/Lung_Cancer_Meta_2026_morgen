...
get_lineage_markers <- function(params) {
  ...
  unique(all_genes)  # preserves first-occurrence order
}
...
#' Works for ANY gene list (GPRIN2 today, something else tomorrow)
#'
#' @param sobj Seurat object
...
analyze_project_genes <- function(sobj, genes, plot_dir, report_dir, prefix = "05_project_genes") {

  genes_present <- genes[genes %in% rownames(sobj)]
  genes_missing <- setdiff(genes, rownames(sobj))

  if (length(genes_missing) > 0) {
    log_msg(sprintf("  Project genes NOT in data: %s", paste(genes_missing, collapse = ", ")), "warn")
  }
  ...
  for (gene in genes_present) {
    ...
    group_summary <- do.call(rbind, lapply(sort(unique(md$group)), function(grp) {
      ...
      e <- expr[cells]
      data.frame(gene = gene, group = grp,
                 total_umi = sum(e), n_pos = sum(e > 0), n_total = length(e),
...
cat("[init] workflow/scrna/functions/annotation_utils.R loaded\n")
